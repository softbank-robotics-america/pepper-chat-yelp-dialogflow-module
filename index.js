    class YelpPepperChat {
        constructor(yelpModule, response, pepperChatResponses, config) {
            // Pepper Chat Library responses required
           if (!('CarouselImage' in pepperChatResponses)) { throw "Yelp Module requires Pepper Chat Library's CarouselImage"}
           if (!('Carousel' in pepperChatResponses)) { throw "Yelp Module requires Pepper Chat Library's Carousel"}
           if (!('FullScreenImage' in pepperChatResponses)) { throw "Yelp Module requires Pepper Chat Library's FullScreenImage"}   
           if (!('BasicText' in pepperChatResponses)) { throw "Yelp Module requires Pepper Chat Library's BasicText"}                          
           if (!('TriggerIntent' in pepperChatResponses)) { throw "Yelp Module requires Pepper Chat Library's TriggerIntent"}                  
           if (!('PepperResponse' in pepperChatResponses)) { throw "Yelp Module requires Pepper Chat Library's PepperResponse"}
           // Yelp API Key
           if (!('yelpApiKey' in config)) { throw "Please provide Yelp API key as property 'yelpApiKey' in config object" }
           // Google Maps API key required
           if (!('googleMapsApiKey' in config)) { throw "Please provide Google Static Maps API key as property 'googleMapsApiKey' in config object" }
           // Geocoder API key optional -- allows for impromptu searching of other cities
           if ('geocoder' in config) { 
               if (!('geocoderApiKey' in config)) { throw "Please provide Geocoder API key as property 'geocoderApiKey' in config object" }    
    ...
    localBusinessHandler({action, contexts, parameters}, session, { local, init }) {
        ...
        console.log("lat:", lat);
                ...
                if (which_city) {
                    ...
                    try {
                        this.geocoder.geocode(which_city)
                            .then(function(resp) {
                                ...
                                getYelpRecommendations(search_terms, filter_by, latitude, longitude).then(carousel => carousel.send(this.response));
                            })
                            .catch(function(err) {
                                console.log(err);
                            });
                    ...
                    getYelpRecommendations(search_terms, filter_by, lat, long).then(carousel => carousel.send(this.response));
                }
            },
            'yelp.search.business_selected': () => {
                try {
                    ...
                    carouselResponse.setContext({ name:"yelp_business_selected",lifespan:2,parameters:yelpBizMenuParameters }, session);                    
                    carouselResponse.send(this.response);
                } catch(err){
                    console.log(err);
                    let errorResponse = new PepperResponse("What else would you like to talk about?");
                    errorResponse.send(this.response);
                }
            },
            'yelp.search.business_selected.map_selected': () => {
                ...
                mapThenReturnToBizMenu.setContext({name: "yelp_biz_visited", lifespan: 2, parameters: {visited: true}}, session);
                mapThenReturnToBizMenu.send(this.response);
            },
            'yelp.search.business_selected.more_info': () => {
                ...
                moreBizInfoResponse.setContext({name: "yelp_biz_visited", lifespan: 2, parameters: {visited: true}}, session);
                moreBizInfoResponse.send(this.response);
            }
        };
